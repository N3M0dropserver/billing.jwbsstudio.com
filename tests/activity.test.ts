...
import { pixelBytes, pixelUrl } from '~/lib/mail/tracking';
...
describe('the tracking pixel', () => {
  it('is a valid 1x1 GIF', () => {
    const gif = pixelBytes();
    expect(gif.byteLength).toBe(42);
    expect(String.fromCharCode(...gif.subarray(0, 6))).toBe('GIF89a');
    // Width and height, little-endian, at bytes 6-9.
    expect(gif[6]! | (gif[7]! << 8)).toBe(1);
    expect(gif[8]! | (gif[9]! << 8)).toBe(1);
  });

  it('builds a URL from the send token', () => {
    expect(pixelUrl('https://billing.example.com', 'tok-123')).toBe(
      'https://billing.example.com/e/tok-123.gif',
    );
  });

  it('does not double the slash when the base URL has a trailing one', () => {
    expect(pixelUrl('https://billing.example.com/', 'tok')).toBe(
      'https://billing.example.com/e/tok.gif',
    );
  });

  it('escapes a token that would otherwise change the path', () => {
    expect(pixelUrl('https://x.test', 'a/b')).toBe('https://x.test/e/a%2Fb.gif');
  });
...
describe('the email carries the pixel only when asked', () => {
  ...
  it('embeds the pixel when a URL is given', () => {
    const mail = invoiceEmail({ ...data, trackingPixelUrl: 'https://billing.example.com/e/tok.gif' });
    expect(mail.html).toContain('src="https://billing.example.com/e/tok.gif"');
  ...
  it('never puts the tracking URL in the plain-text part', () => {
    const mail = invoiceEmail({ ...data, trackingPixelUrl: 'https://billing.example.com/e/tok.gif' });
    expect(mail.text).not.toContain('/e/tok');
  });
});
